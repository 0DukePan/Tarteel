...
export const registrationValidation = [
  body("parent.fatherFirstName")
    .trim()
    .isLength({ min: 2, max: 50 })
    .withMessage("Father first name must be between 2 and 50 characters"),
  body("parent.fatherLastName")
    .trim()
    .isLength({ min: 2, max: 50 })
    .withMessage("Father last name must be between 2 and 50 characters"),
  body("parent.fatherPhone")
    .trim()
    .matches(/^[+]?[1-9][\d]{0,15}$/)
    .withMessage("Please enter a valid phone number"),
  body("parent.fatherEmail")
    .trim()
    .isEmail()
    .normalizeEmail()
    .withMessage("Please enter a valid email address"),
  body("parent.motherFirstName")
    .optional()
    .trim()
    .isLength({ max: 50 })
    .withMessage("Mother first name cannot exceed 50 characters"),
  body("parent.motherLastName")
    .optional()
    .trim()
    .isLength({ max: 50 })
    .withMessage("Mother last name cannot exceed 50 characters"),
  body("parent.motherPhone")
    .optional({ checkFalsy: true })
    .trim()
    .matches(/^[+]?[1-9][\d]{0,15}$/)
    .withMessage("Please enter a valid phone number"),
  body("parent.motherEmail")
    .optional({ checkFalsy: true })
    .trim()
    .isEmail()
    .normalizeEmail()
  ...
  body("student.classId").isUUID().withMessage("Please select a valid class"),
];
...
export const classValidation = [
  ...
  body("startTime")
    ...
    .withMessage("Please enter a valid start time (HH:MM)"),
  body("endTime")
    .matches(/^([0-1]?[0-9]|2[0-3]):[0-5][0-9]$/)
    .withMessage("Please enter a valid end time (HH:MM)"),
